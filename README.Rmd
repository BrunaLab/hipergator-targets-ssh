...
2.  This project uses `renv` to manage R package dependencies.
...
5.  Edit the `slurm_clustermq.tmpl` file if needed (but don't touch the wildcards in double curly braces) and then copy it to HiperGator with `scp slurm_clustermq.tmpl username@hpg.rc.ufl.edu:slurm_clustermq.tmpl`.
    ...
    \*\* NOTE: \*\* While you're logged into HiperGator, you can also check that all the packages your project uses are also installed on the cluster.
...
Currently (as of 10-18-2021) there is a bug with R version 4.1+ on HiperGator that affects this workflow.
